...
function Toggle({ id }) {
    ...
    e.stopPropagation()
		...
		// if there is no Id || the opened window is different from the current one
		// then open the menu
		// else close it
...
function List({ id, children }) {
	...
	// else then open this window, with createPortal, because this is hovering too
	return createPortal(
		// position determines where the window will be displayed
...
function Button({ children, icon, onClick }) {
	...
	function handleClick() {
		// optional chaining to make sure onClick exist
		onClick?.();
		close();
	}
